...
class MemberController extends Controller
{
    ...
    public function show(Member $member): \Illuminate\View\View
    {
        $member->load([
            'region',
            'positions',
            'savingsAccount.transactions' => fn ($q) => $q->latest('transaction_date')->limit(10),
            'shareAccount.transactions' => fn ($q) => $q->latest('transaction_date')->limit(10),
            'loans' => fn ($q) => $q->latest(),
            'nextOfKins',
        ]);
    ...
    public function update(Request $request, Member $member): \Illuminate\Http\RedirectResponse
    {
        $validated = $request->validate([
            'region_id' => 'required|exists:regions,id',
            'staff_id' => 'required|string|unique:members,staff_id,' . $member->id,
            'first_name' => 'required|string|max:255',
            'last_name' => 'required|string|max:255',
            'middle_name' => 'nullable|string|max:255',
    ...
    public function importStore(Request $request): \Illuminate\Http\RedirectResponse
    {
        ...
        try {
            $results = Excel::import(new MemberImport, $request->file('import_file'));
    ...
    public function downloadTemplate(): \Symfony\Component\HttpFoundation\StreamedResponse
    {
        ...
        $callback = function () {
            ...
            fputcsv($file, [
                ...
                'address', 'state_of_origin', 'nin', 'grade_level', 'monthly_salary', 'status',
            ]);
            fputcsv($file, [
