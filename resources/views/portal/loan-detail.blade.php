    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                <a href="{{ route('portal.loans') }}" class="text-slate-500 hover:text-slate-700">&larr;</a>
                <h2 class="text-2xl font-bold text-[#0F172A]">{{ $loan->loan_number }}</h2>
                @php
                    $statusStyles = [
            ...
            @if (in_array($loan->status, ['disbursed', 'repaying', 'completed', 'defaulted']))
                <a href="{{ route('receipts.loan-statement', $loan) }}" target="_blank" class="inline-flex items-center gap-1.5 text-slate-500 hover:text-slate-700 px-3 py-2 rounded-[10px] text-sm transition">
        ...
        @if ($loan->isTopup() && $loan->parentLoan)
            <div class="bg-indigo-50 border border-indigo-200 rounded-[16px] p-4 flex items-start gap-3">
        ...
        @if ($loan->status === 'rejected' && $loan->rejection_reason)
            <div class="bg-red-50 border border-red-200 rounded-[16px] p-4 flex items-start gap-3">
        ...
        @if ($loan->isOverdue())
            <div class="bg-gradient-to-r from-red-500 to-red-600 rounded-[16px] p-4 flex items-center gap-3 text-white">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white rounded-[16px] shadow-sm border border-slate-200 p-6">
                    ...
                    <dl class="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                        <div>
                            <dt class="text-slate-500 text-xs">Loan Number</dt>
                            <dd class="font-mono font-medium">{{ $loan->loan_number }}</dd>
                        </div>
                        <div>
                            <dt class="text-slate-500 text-xs">Loan Product</dt>
                            <dd class="font-medium">{{ $loan->loanProduct->name ?? ucfirst($loan->type) }}</dd>
                        </div>
                        <div>
                            <dt class="text-slate-500 text-xs">Loan Amount</dt>
                            <dd class="font-bold text-[#0F172A]">₦{{ number_format($loan->amount, 2) }}</dd>
                        </div>
                        <div>
                            <dt class="text-slate-500 text-xs">Interest Rate</dt>
                            <dd class="font-medium">{{ $loan->interest_rate }}%</dd>
                        </div>
                        <div>
                            <dt class="text-slate-500 text-xs">Tenure</dt>
                            <dd class="font-medium">{{ $loan->tenure_months }} months</dd>
                        </div>
                        <div>
                            <dt class="text-slate-500 text-xs">Monthly Repayment</dt>
                            <dd class="font-medium">₦{{ number_format($loan->monthly_repayment, 2) }}</dd>
                        </div>
                        <div>
                            <dt class="text-slate-500 text-xs">Total Repaid</dt>
                            <dd class="font-medium text-green-600">₦{{ number_format($loan->total_repaid, 2) }}</dd>
                        </div>
                        <div>
                            <dt class="text-slate-500 text-xs">Outstanding</dt>
                            <dd class="font-bold text-orange-600">₦{{ number_format($loan->outstanding, 2) }}</dd>
                        </div>
                        <div>
                            <dt class="text-slate-500 text-xs">Application Date</dt>
                            <dd class="font-medium">{{ $loan->application_date?->format('d M Y') }}</dd>
                        </div>
                        <div>
                            <dt class="text-slate-500 text-xs">Approval Date</dt>
                            <dd class="font-medium">{{ $loan->approval_date?->format('d M Y') ?? 'Pending' }}</dd>
                        </div>
                        <div>
                            <dt class="text-slate-500 text-xs">Disbursement Date</dt>
                            <dd class="font-medium">{{ $loan->disbursement_date?->format('d M Y') ?? 'N/A' }}</dd>
                        </div>
                        <div>
                            <dt class="text-slate-500 text-xs">Maturity Date</dt>
                    ...
                    @if ($loan->purpose)
                        <div class="mt-4 pt-4 border-t border-slate-200">
                            ...
                            <p class="text-sm text-slate-700">{{ $loan->purpose }}</p>
                        </div>
                    @endif
                </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-sm">
                                <thead class="border-b border-slate-200">
                                    <tr>
                                        <th class="text-left px-3 py-2 font-medium text-slate-600 text-xs">Date</th>
                                        <th class="text-left px-3 py-2 font-medium text-slate-600 text-xs">Reference</th>
                                        <th class="text-right px-3 py-2 font-medium text-slate-600 text-xs">Amount</th>
                                        <th class="text-right px-3 py-2 font-medium text-slate-600 text-xs">Principal</th>
                                        <th class="text-right px-3 py-2 font-medium text-slate-600 text-xs">Interest</th>
                                        <th class="text-right px-3 py-2 font-medium text-slate-600 text-xs">Balance After</th>
                                        <th class="text-right px-3 py-2 font-medium text-slate-600 text-xs">Action</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-200">
                                    @foreach ($loan->repayments as $repayment)
                                        <tr>
                                            <td class="px-3 py-2 text-xs text-slate-600">{{ $repayment->payment_date->format('d M Y') }}</td>
                                            <td class="px-3 py-2 font-mono text-[11px] text-slate-500">{{ $repayment->reference }}</td>
                                            <td class="px-3 py-2 text-right text-xs font-medium text-green-600">₦{{ number_format($repayment->amount, 2) }}</td>
                                            <td class="px-3 py-2 text-right text-xs text-slate-600">₦{{ number_format($repayment->principal_portion, 2) }}</td>
                                            <td class="px-3 py-2 text-right text-xs text-slate-600">₦{{ number_format($repayment->interest_portion, 2) }}</td>
                                            <td class="px-3 py-2 text-right text-xs font-medium text-orange-600">₦{{ number_format($repayment->outstanding_after, 2) }}</td>
                                            <td class="px-3 py-2 text-right">
                                                <a href="{{ route('receipts.loan-repayment', $repayment) }}" target="_blank" class="text-[11px] text-slate-500 hover:text-slate-700">
                ...
                @if ($loan->schedules->isNotEmpty())
                    <div class="bg-white rounded-[16px] shadow-sm border border-slate-200 p-6">
                        <h3 class="text-lg font-semibold text-[#0F172A] mb-4">Repayment Schedule</h3>
                        <div class="overflow-x-auto">
                            <table class="w-full text-sm">
                                <thead class="border-b border-slate-200">
                                    <tr>
                                        <th class="text-left px-3 py-2 font-medium text-slate-600 text-xs">#</th>
                                        <th class="text-left px-3 py-2 font-medium text-slate-600 text-xs">Due Date</th>
                                        <th class="text-right px-3 py-2 font-medium text-slate-600 text-xs">Principal</th>
                                        <th class="text-right px-3 py-2 font-medium text-slate-600 text-xs">Interest</th>
                                        <th class="text-right px-3 py-2 font-medium text-slate-600 text-xs">Total</th>
                                        <th class="text-left px-3 py-2 font-medium text-slate-600 text-xs">Status</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-200">
                                    @foreach ($loan->schedules as $schedule)
                                        <tr>
                                            <td class="px-3 py-2 text-xs text-slate-500">{{ $schedule->installment_number }}</td>
                                            <td class="px-3 py-2 text-xs text-slate-600">{{ $schedule->due_date->format('d M Y') }}</td>
            ...
            <div class="space-y-6">
                ...
                @endphp
                <div class="bg-white rounded-[16px] shadow-sm border border-slate-200 p-6">
                    <h3 class="text-lg font-semibold text-[#0F172A] mb-4">Repayment Progress</h3>
                ...
                @if ($loan->guarantors->isNotEmpty())
                    <div class="bg-white rounded-[16px] shadow-sm border border-slate-200 p-6">
                        <h3 class="text-lg font-semibold text-[#0F172A] mb-3 flex items-center gap-2">
                        ...
                        <div class="space-y-2">
                            @foreach ($loan->guarantors as $guarantor)
                                ...
                                @endphp
                                <div class="flex items-center justify-between p-2 rounded-[10px] bg-slate-50">
                                    <div class="flex items-center gap-2">
                                        <div class="w-7 h-7 rounded-full bg-slate-200 flex items-center justify-center text-[10px] font-bold text-slate-600">
                                            {{ strtoupper(substr($guarantor->member->first_name ?? '?', 0, 1) . substr($guarantor->member->last_name ?? '?', 0, 1)) }}
                                        </div>
                                        <div>
                                            <p class="text-xs font-medium text-[#0F172A]">{{ $guarantor->member->first_name ?? 'N/A' }} {{ $guarantor->member->last_name ?? '' }}</p>
                                            <p class="text-[10px] text-slate-500">{{ $guarantor->member->staff_id_display ?? '' }}</p>
                                        </div>
                                    </div>
