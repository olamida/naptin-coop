<x-portal-layout title="Checkout">
    <div class="space-y-6">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('portal.cart') }}" class="text-slate-500 hover:text-slate-700">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-4">
                <div class="bg-white rounded-[16px] shadow-sm border border-slate-200 overflow-hidden">
                    ...
                    <table class="w-full text-sm">
                        <thead class="bg-slate-50 border-b border-slate-200">
                            <tr>
                                <th class="text-left px-5 py-2.5 text-xs font-semibold text-slate-500 uppercase tracking-wider">Product</th>
                                <th class="text-right px-5 py-2.5 text-xs font-semibold text-slate-500 uppercase tracking-wider">Qty</th>
                ...
                <form method="POST" action="{{ route('portal.checkout.process') }}" class="bg-white rounded-[16px] shadow-sm border border-slate-200 p-5 space-y-4">
                    @csrf

                    <div class="bg-slate-50 border border-slate-200 rounded-[12px] p-3 flex items-center gap-3">
                        <span class="material-symbols-outlined text-blue-600">person</span>
                        <div>
                            <p class="text-sm font-medium text-[#0F172A]">Ordering as: {{ $member->full_name }}</p>
                            <p class="text-xs text-blue-600">{{ $member->staff_id_display }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-3 pt-3 border-t border-slate-200">
                        ...
                        <a href="{{ route('portal.cart') }}" class="text-sm text-slate-500 hover:text-slate-700">Back to Cart</a>
                    </div>
                </form>
            </div>
