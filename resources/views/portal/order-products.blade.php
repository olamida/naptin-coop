    ...
    <div class="space-y-6" x-data="shopApp()">
                ...
                <div class="flex flex-col sm:flex-row gap-3">
                    <div class="flex-1 relative">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Search products..."
                               class="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none">
                    </div>
                    <div class="flex gap-2 items-center">
                        <select name="sort" class="px-3 py-2.5 border border-gray-300 rounded-lg text-sm bg-white focus:ring-2 focus:ring-blue-500 outline-none">
                        ...
                        <div class="flex items-center gap-1">
                            <input type="number" name="min_price" value="{{ request('min_price') }}" placeholder="Min"
                                   class="w-20 px-2 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" step="100">
                        ...
                        @if (request()->hasAny(['search', 'sort', 'min_price', 'max_price']))
                            <a href="{{ route('portal.products') }}" class="text-gray-500 hover:text-gray-700 px-2 py-2 text-sm">Clear</a>
                        @endif
                    </div>
                </div>
            </form>
        ...
        @if (request()->hasAny(['search', 'min_price', 'max_price']))
            <div class="flex items-center gap-2 text-sm text-gray-600">
                <span class="material-symbols-outlined text-lg text-gray-400">filter_alt</span>
        ...
        @if ($products->count() > 0)
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                @foreach ($products as $product)
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg transition-all duration-200 group flex flex-col">
                        <div class="relative h-48 bg-gradient-to-br from-gray-100 to-gray-50 flex items-center justify-center overflow-hidden">
                        ...
                        <div class="p-4 flex flex-col flex-1">
                            ...
                            <p class="text-xs text-gray-500 line-clamp-2 mb-3 flex-1">{{ $product->description ?? 'No description available.' }}</p>
                            <div class="flex items-end justify-between mb-3">
                                <span class="text-lg font-bold text-gray-900">₦{{ number_format($product->unit_price, 2) }}</span>
                                <span class="text-[11px] text-gray-400">Stock: {{ $product->stock_quantity }}</span>
                            </div>
                            <div x-data="{ added: false, loading: false }">
                                ...
                                ">
                                    ...
                                    <button type="submit" :disabled="loading || added"
                                            :class="added ? 'bg-green-500 hover:bg-green-500' : 'bg-blue-600 hover:bg-blue-700'"
                                            class="w-full text-white px-3 py-2.5 rounded-xl text-sm font-medium transition flex items-center justify-center gap-1.5 disabled:opacity-60 disabled:cursor-not-allowed">
        ...
        @else
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-16 text-center">
                <span class="material-symbols-outlined text-6xl text-gray-200 mb-4 block">search_off</span>
                <p class="text-gray-500 text-lg mb-2">No products found</p>
                <p class="text-sm text-gray-400 mb-6">Try adjusting your search or filter criteria.</p>
