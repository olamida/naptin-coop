<x-portal-layout title="Guarantor Requests">
    <div class="space-y-6">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('portal.dashboard') }}" class="text-slate-500 hover:text-slate-700">&larr;</a>
            <h2 class="text-2xl font-bold text-[#0F172A]">My Guarantor Requests</h2>
        </div>
        ...
        <div class="space-y-4">
                    ...
                    <div class="p-5">
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-slate-500">
                                        {{ $request->loan->member->staff_id_display ?? 'N/A' }}
                        ...
                        <div class="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3">
                            <div class="bg-slate-50 rounded-[10px] p-3">
                                <p class="text-[10px] text-slate-500 uppercase">Loan Number</p>
                            ...
                            <div class="bg-slate-50 rounded-[10px] p-3">
                                <p class="text-[10px] text-slate-500 uppercase">Monthly Repayment</p>
                        ...
                        <div class="px-5 py-3 bg-slate-50 border-t border-slate-200">
                            <div class="flex items-center gap-3 flex-wrap">
                                ...
                                @if ($request->accept_token)
                                    <button onclick="navigator.clipboard.writeText('{{ route('guarantee.show', $request->accept_token) }}').then(() => { this.textContent='Copied!'; setTimeout(() => this.textContent='Copy Link', 2000); })"
                                        class="ml-auto text-xs text-indigo-600 hover:text-indigo-800 underline underline-offset-2">
                                        Copy Link
                                    </button>
                                @endif
                            </div>
            ...
            @empty
                <div class="bg-white rounded-[16px] shadow-sm border border-slate-200 p-12 text-center">
                    <span class="material-symbols-outlined text-slate-300 text-5xl mb-3">group_add</span>
                    <h3 class="text-lg font-medium text-slate-700">No Guarantor Requests</h3>
                    <p class="text-sm text-slate-500 mt-1">You have not been asked to guarantee any loans.</p>
                </div>
            @endforelse
        </div>
