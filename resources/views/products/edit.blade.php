        ...
        <div class="bg-white rounded-[16px] shadow-sm border border-slate-200 p-6">
            <form method="POST" action="{{ route('products.update', $product) }}" enctype="multipart/form-data" class="space-y-5">
                ...
                <div x-data="{ preview: '{{ $product->image_url }}', removed: false }" class="text-center">
                    ...
                    <template x-if="!preview || removed">
                        <div>
                            <div class="w-32 h-32 rounded-[16px] bg-slate-100 flex items-center justify-center mx-auto mb-3 border-2 border-dashed border-slate-300">
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="cost_price" value="{{ old('cost_price', $product->cost_price) }}" step="0.01" min="0"
                               class="w-full px-3 py-2 border border-slate-300 rounded-[10px] text-sm focus:ring-2 focus:ring-blue-500 outline-none">
                        <p class="text-[11px] text-slate-400 mt-1">Defaults to unit price.</p>
                    </div>
                    <div>
