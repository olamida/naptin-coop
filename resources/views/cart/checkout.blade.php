<x-app-layout title="Checkout">
    <div class="space-y-6">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('cart.index') }}" class="text-gray-500 hover:text-gray-700">&larr;</a>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-4">
                ...
                <form method="POST" action="{{ route('cart.process') }}" class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-4">
                    @csrf
                    <div>
                        ...
                        <select name="member_id" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none">
                            ...
                            @foreach ($members as $member)
                                <option value="{{ $member->id }}" {{ ($memberId ?? null) == $member->id ? 'selected' : '' }}>{{ $member->full_name }} ({{ $member->staff_id }})</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="flex items-center gap-3 pt-3 border-t border-gray-100">
                        ...
                        <a href="{{ route('cart.index') }}" class="text-sm text-gray-500 hover:text-gray-700">Back to Cart</a>
                    </div>
                </form>
            </div>
