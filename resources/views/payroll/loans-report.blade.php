<x-app-layout title="Loan Repayment Report">
    <div class="space-y-6">
        <div class="flex items-center justify-between no-print">
            <div class="flex items-center gap-3">
                <a href="{{ route('payroll.show', $payroll) }}" class="text-gray-500 hover:text-gray-700">&larr;</a>
                <div>
                    <h2 class="text-2xl font-bold text-gray-800">Loan Repayment Report</h2>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="text-right px-4 py-3 font-medium text-gray-600">Expected Deduction</th>
                        <th class="text-right px-4 py-3 font-medium text-gray-600">Actual Deduction</th>
                        <th class="text-left px-4 py-3 font-medium text-gray-600">Status</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    @foreach ($payroll->deductions->where('expected_loan_repayment', '>', 0) as $i => $d)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-right font-medium">₦{{ number_format($d->expected_loan_repayment, 2) }}</td>
                            <td class="px-4 py-3 text-right {{ $d->actual_loan_repayment > 0 ? 'text-green-700 font-semibold' : '' }}">₦{{ number_format($d->actual_loan_repayment, 2) }}</td>
                            <td class="px-4 py-3">
                ...
                <tfoot class="bg-gray-50 border-t border-gray-200 font-semibold">
                    <tr>
                        ...
                        <td class="px-4 py-3 text-right">₦{{ number_format($payroll->total_loan_repayments, 2) }}</td>
                        <td class="px-4 py-3 text-right text-green-700">₦{{ number_format($payroll->deductions->sum('actual_loan_repayment'), 2) }}</td>
                        <td></td>
                    </tr>
                </tfoot>
            </table>
