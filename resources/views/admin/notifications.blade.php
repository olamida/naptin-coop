        ...
        <div class="bg-white rounded-[16px] shadow-sm border border-slate-200 overflow-hidden">
            @forelse ($notifications as $notification)
                @php
                    ...
                    $actionUrl = \App\Support\NotificationLinks::actionUrl($data, 'admin');
                @endphp
                @if ($actionUrl)
                ...
                @endif
                    ...
                    <div class="flex items-center gap-2 flex-shrink-0">
                        @if ($isUnread)
                            <form method="POST" action="{{ route('admin.notifications.read', $notification->id) }}" @click.prevent.stop>
                                @csrf
                ...
                @endif
            @empty
                <x-empty-state icon="notifications_off" title="No notifications yet"
                    message="System events and alerts will appear here as they happen." />
            @endforelse
        </div>
