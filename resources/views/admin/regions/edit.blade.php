<x-app-layout>
    <div class="max-w-2xl mx-auto space-y-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Edit Regional Center</h2>
            <p class="text-gray-500 text-sm mt-1">Update {{ $region->name }} details</p>
        </div>
        ...
        <form action="{{ route('admin.regions.update', $region) }}" method="POST" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-5">
            @csrf @method('PUT')
            ...
            <div>
                ...
                <textarea name="address" id="address" rows="2"
                          class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">{{ old('address', $region->address) }}</textarea>
            </div>
