<x-app-layout title="Edit Member">
    <div class="max-w-3xl space-y-6">
        ...
        <div class="flex items-center gap-3">
            ...
            <h2 class="text-2xl font-bold text-[#0F172A]">Edit Member</h2>
        </div>
        ...
        <form method="POST" action="{{ route('members.update', $member) }}" enctype="multipart/form-data" class="bg-white rounded-[16px] shadow-sm border border-slate-200 p-6 space-y-6">
            @csrf
            @method('PUT')
                        ...
                        <div x-data="{ preview: '{{ $member->photo_url }}', removed: false }" class="text-center">
                            <template x-if="preview && !removed">
                                <div class="mb-3 relative inline-block">
                                    <img :src="preview" class="w-28 h-28 rounded-full object-cover border-4 border-slate-200">
                            ...
                            <template x-if="!preview || removed">
                                <div>
                                    <div class="w-28 h-28 rounded-full bg-slate-100 flex items-center justify-center mx-auto mb-3 border-4 border-dashed border-slate-300">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-slate-700 mb-1">Address</label>
                            <textarea name="address" rows="2" class="w-full px-3 py-2 border border-slate-300 rounded-[10px] text-sm focus:ring-2 focus:ring-blue-500 outline-none">{{ old('address', $member->address) }}</textarea>
                    </div>
                </div>
            </fieldset>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Staff ID *</label>
                            <input type="number" name="staff_id" value="{{ old('staff_id', $member->staff_id) }}" required
                                class="w-full px-3 py-2 border border-slate-300 rounded-[10px] text-sm focus:ring-2 focus:ring-blue-500 outline-none">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="text-[11px] text-slate-400 mt-1">Member's preferred amount for monthly payroll deduction</p>
                    </div>
                    <div>
            ...
            <div class="flex items-center gap-3 pt-4 border-t border-slate-200">
                ...
                <a href="{{ route('members.show', $member) }}" class="text-sm text-slate-500 hover:text-slate-700">Cancel</a>
            </div>
        </form>
    </div>
</x-app-layout>
