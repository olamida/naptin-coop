<x-public-layout title="Shop">
    ...
    <div class="bg-gray-50 border-b border-gray-200">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <h1 class="text-2xl font-bold text-gray-800">Cooperative Shop</h1>
            <p class="text-sm text-gray-500 mt-1">Browse our products available for members</p>
        </div>
    </div>

    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {{-- Search Bar --}}
        <form method="GET" class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-8">
            <div class="flex flex-wrap gap-3 items-end">
                ...
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition">Search</button>
                @if (request()->hasAny(['search', 'in_stock']))
                    <a href="{{ route('shop') }}" class="text-sm text-gray-500 hover:underline py-2">Clear</a>
                @endif
            </div>
        </form>
                    ...
                    <div class="p-4">
                        ...
                        @if ($product->stock_quantity > 0 && $product->enabled)
                            @auth
                                ...
                                @else
                                    <a href="{{ route('login') }}" class="w-full block text-center bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded-lg text-xs font-medium transition">
                                        Login to Order
                                    </a>
                                @endif
                            @else
