<x-app-layout title="Deposit">
    <div class="max-w-xl space-y-6">
        ...
        <div class="flex items-center gap-3">
            ...
            <h2 class="text-2xl font-bold text-gray-800">Savings Deposit</h2>
        </div>
        ...
        <form method="POST" action="{{ route('savings.deposit.store') }}" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-5" x-data="evidenceUpload()">
            @csrf

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Member *</label>
                <div x-data="memberSearch(@json($members->map(fn($m) => ['id' => $m->id, 'first_name' => $m->first_name, 'last_name' => $m->last_name, 'staff_id' => $m->staff_id, 'account_number' => $m->savingsAccount?->account_number ?? ''])))" x-init="init()">
                    <div class="relative">
                        ...
                        <div x-show="showDropdown && filteredMembers.length > 0" x-transition
                            ...
                            <template x-for="m in filteredMembers" :key="m.id">
                                <div @click="selectMember(m)" class="px-3 py-2 cursor-pointer hover:bg-blue-50 text-sm border-b border-gray-50 last:border-0"
                                    ...
                                    <span class="text-xs text-gray-400 ml-1" x-text="'(' + (m.account_number || 'No Account') + ')'"></span>
                                </div>
                            </template>
                        </div>
                    </div>
            ...
            <div>
                ...
                <div class="border-2 border-dashed border-gray-200 rounded-lg p-4 text-center hover:border-blue-300 transition">
                    <template x-if="!preview">
                        <label class="cursor-pointer">
                            ...
                            <p class="text-xs text-gray-500 mt-1">Click to upload receipt or payment proof (image)</p>
                ...
                <p class="text-[11px] text-gray-400 mt-1">Optional. Upload payment receipt or transfer confirmation.</p>
            </div>
