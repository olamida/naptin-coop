<x-app-layout title="Pending Approvals">
    <div class="space-y-6">
        ...
        <div class="flex items-center gap-1 border-b border-gray-200">
            ...
            <a href="{{ route('savings.pending-withdrawals') }}" class="px-4 py-2.5 text-sm font-medium border-b-2 border-blue-600 text-blue-600 -mb-px flex items-center gap-1">
        ...
        @if ($pendingDeposits->count() > 0)
            <div class="flex items-center gap-2 mb-2">
                ...
                <h3 class="font-semibold text-gray-800">Pending Deposits ({{ $pendingDeposits->count() }})</h3>
            </div>
                    ...
                    <div class="flex items-start justify-between gap-4">
                        <div class="flex items-start gap-4 flex-1 min-w-0">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                @endif
                                <p class="text-xs text-gray-400 mt-1 font-mono">{{ $txn->reference }}</p>
                            </div>
                        </div>
                        <div class="flex items-center gap-2 shrink-0">
                            <form method="POST" action="{{ route('savings.deposits.confirm', $txn) }}">
                                ...
                                <button type="submit" onclick="return confirm('Confirm this deposit of &#8358;{{ number_format($txn->amount, 2) }}? This will add to the member\'s balance.')"
                                    class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition flex items-center gap-1.5">
                                    <span class="material-symbols-outlined text-[16px]">check</span>
                                    Confirm
                                </button>
                            </form>
                            <button onclick="document.getElementById('rejectDepositModal{{ $txn->id }}').classList.remove('hidden')"
                                class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition flex items-center gap-1.5">
                ...
                <div id="rejectDepositModal{{ $txn->id }}" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6">
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <div>
                                <h3 class="text-lg font-semibold text-gray-800">Reject Deposit</h3>
                                <p class="text-sm text-gray-500">&#8358;{{ number_format($txn->amount, 2) }} — {{ $txn->savingsAccount->member->first_name ?? '' }} {{ $txn->savingsAccount->member->last_name ?? '' }}</p>
                            </div>
                        </div>
                        <form method="POST" action="{{ route('savings.deposits.reject', $txn) }}">
                            @csrf
                            <div class="mb-4">
                                <label for="rejection_reason{{ $txn->id }}" class="block text-sm font-medium text-gray-700 mb-1">Reason for rejection <span class="text-red-500">*</span></label>
                                <textarea name="rejection_reason" id="rejection_reason{{ $txn->id }}" rows="3" required class="w-full border border-gray-200 rounded-lg text-sm p-3 focus:ring-2 focus:ring-red-500 focus:border-red-500" placeholder="Enter reason..."></textarea>
                            </div>
                            <div class="flex items-center justify-end gap-2">
                                <button type="button" onclick="document.getElementById('rejectDepositModal{{ $txn->id }}').classList.add('hidden')" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-800 transition">Cancel</button>
                                <button type="submit" onclick="return confirm('Reject this deposit?')" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg text-sm transition">Reject</button>
                            </div>
                        </form>
                    </div>
                </div>
        ...
        @if ($withdrawals->count() > 0)
            <div class="flex items-center gap-2 mb-2 {{ $pendingDeposits->count() > 0 ? 'mt-8' : '' }}">
                ...
                <h3 class="font-semibold text-gray-800">Pending Withdrawals ({{ $withdrawals->count() }})</h3>
            </div>
                    ...
                    <div class="flex items-start justify-between gap-4">
                        <div class="flex items-start gap-4 flex-1 min-w-0">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <dl class="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3 text-sm">
                                    <div>
                                        <dt class="text-gray-500 text-xs">Withdrawal Amount</dt>
                                    ...
                                    <div>
                                        <dt class="text-gray-500 text-xs">Remaining After</dt>
                ...
                <div id="rejectModal{{ $txn->id }}" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6">
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <div>
                                <h3 class="text-lg font-semibold text-gray-800">Reject Withdrawal</h3>
                                <p class="text-sm text-gray-500">&#8358;{{ number_format($txn->amount, 2) }} — {{ $txn->savingsAccount->member->first_name ?? '' }} {{ $txn->savingsAccount->member->last_name ?? '' }}</p>
                            </div>
                        </div>
                        <form method="POST" action="{{ route('savings.withdrawals.reject', $txn) }}">
                            ...
                            <div class="flex items-center justify-end gap-2">
                                <button type="button" onclick="document.getElementById('rejectModal{{ $txn->id }}').classList.add('hidden')" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-800 transition">Cancel</button>
                                <button type="submit" onclick="return confirm('Reject this withdrawal?')" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg text-sm transition">Reject</button>
                            </div>
                        </form>
                    </div>
                </div>
        ...
        @if ($withdrawals->count() === 0 && $pendingDeposits->count() === 0)
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
                ...
                <h3 class="text-lg font-semibold text-gray-800 mb-1">All caught up!</h3>
                <p class="text-sm text-gray-500">No pending requests to review.</p>
